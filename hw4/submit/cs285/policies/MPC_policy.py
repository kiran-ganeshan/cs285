...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        if self.sample_strategy == 'random' \
            ...
            random_action_sequences = np.random.uniform(self.low, self.high, size=shape)
            ...
            for i in range(self.cem_iterations):
                shape = (num_sequences, horizon, self.ac_dim)
                if i == 0:
                    actions = np.random.uniform(self.low, self.high, size=shape)
                else:
                    actions = np.random.normal(mean, std, size=shape)
                values = self.evaluate_candidate_sequences(actions, obs)
                idx = np.argpartition(values, -self.cem_num_elites)[-self.cem_num_elites:]
                actions = actions[idx, ...]
                mean = actions.mean(0)
                std = actions.std(0)
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        return reward / len(self.dyn_models)
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        #       action sequence.
        for ac in candidate_action_sequences.transpose(1, 0, 2):
            reward, _ = self.env.get_reward(obs, ac)
