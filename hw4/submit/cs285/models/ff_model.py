...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = (obs_unnormalized - obs_mean) / (obs_std + 1e-9)
        acs_normalized = (acs_unnormalized - acs_mean) / (acs_std + 1e-9)

        # predicted change in obs
        concatenated_input = torch.cat([obs_normalized.squeeze(), acs_normalized], dim=-1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        data_stat = {k: ptu.from_numpy(v) for k, v in data_statistics.items()}
        target = (next_obs - obs - data_stat['delta_mean']) / data_stat['delta_std']# TODO(Q1) compute the normalized target for the model.
